...
def filtering_minutiae(minutiae:np.ndarray,mask:np.ndarray) :
    #cv2.distanceTransform 함수는 입력으로 주어진 이진 이미지에서 객체 경계선까지의 거리를 계산합니다. 
    #여기서는 segmeation mask가 입력으로 들어가고 배경과 전경의 거리를 계산하게 됩니다.
    #이후 이 거리를 통하여 minutiae가 지문의 외각선에 특징점을 잡지 않도록 합니다.
